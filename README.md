## Hey, I'm Shameer

I build production AI systems using LLMs, AI Agents, and RAG. Most of my time goes into FastAPI backends, voice systems, and multi-agent orchestration.

Based in India. Always shipping, not just shipping code.

---

## What I Build

**Voice AI Systems** — Real-time speech conversations with language models. Speech-to-text, LLM reasoning, text-to-speech, all wired together with WebSockets and FastAPI.

**RAG Applications** — Document intelligence using semantic search, vector databases, and LLMs. The kind that actually work in production.

**Agentic AI** — Multi-agent systems that reason, use tools, and coordinate. Currently exploring CrewAI and LangGraph.

**AI Automation** — OCR systems, financial analysis, content generation. Whatever needs intelligence, I'll add a language model and a clean API.

---

## Projects

| Project | Focus |
|---------|-------|
| **Voice AI Agent** | FastAPI • Speech-to-Text • TTS • LLMs • WebSockets • Docker |
| **Multimodal RAG** | Semantic Search • Vector DB • Document Intelligence • FastAPI |
| **Aadhaar OCR Masking** | OCR • Sensitive Data Detection • Computer Vision |
| **Finance AI Assistant** | LLMs • Prompt Engineering • FastAPI • Financial APIs |
| **ISL Recognition** | Graph Neural Networks • MediaPipe • SBERT • PyTorch |
| **Agentic Blog Generator** | CrewAI • Multi-Agent Systems • SEO Automation |
| **Mammogram Classification** | CNN • Medical Imaging • Classification • TensorFlow |

---

## Stack

**Languages** — Python, SQL

**LLMs & AI** — OpenAI, Anthropic, Google Gemini, LangChain, LangGraph, CrewAI

**ML** — TensorFlow, PyTorch, Scikit-learn

**Deep Learning** — Transformers, CNN, RNN, GAN, Graph Neural Networks

**Backend** — FastAPI, REST APIs, WebSockets

**ML Infrastructure** — FAISS, ChromaDB, Vector Embeddings

**Vision** — OpenCV, MediaPipe, OCR

**DevOps** — Docker, GitHub Actions, Linux

**Cloud** — Azure, Google Cloud Platform

---

## Currently Working On

- Advanced agentic AI patterns and multi-agent coordination
- Voice AI systems at scale
- Optimizing RAG pipelines for production
- LangGraph for stateful workflows

---

## Let's Connect

**LinkedIn** — [nitturu-shameer](https://www.linkedin.com/in/nitturu-shameer)

**Email** — nitturushameer37@gmail.com

**GitHub** — [Shameer37](https://github.com/Shameer37)

---

*If you're interested in AI systems, production ML, or just building things that work—feel free to explore my repos. Open to conversations about LLMs, agentic AI, and backend engineering.*
